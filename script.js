...
function attachListeners() {
  ...
  addItemButton.addEventListener('click', () => {
    if (state.mode === 'text' && state.useClassicNumbers) {
      ...
      classicSourceButtons.forEach((b) => b.classList.toggle('active', b.dataset.source === 'custom'));
      toggleClassicInputs();
    }
    addItem();
  });
...
function resetForm() {
  ...
  classicSourceButtons.forEach((btn) => btn.classList.toggle('active', btn.dataset.source === 'numbers'));
  audioPlayer.pause();
